...
def admin_dashboard(db, env_doc: Dict[str, Any], lang: dict):
    ...
    if admin_sub_view == lang["admin_env"]:
        ...
        with st.form("env_form"):
            ...
            env_doc["openai_api_key"] = st.text_input(lang["admin_env_openai_api_key"],
                                                      value=env_doc.get("openai_api_key", ""), type="password")
            env_doc["openai_base_url"] = st.text_input(lang["admin_env_openai_base_url"],
                                                       value=env_doc.get("openai_base_url", ""))
            env_doc["openai_model"] = st.text_input(lang["admin_env_openai_model"],
                                                    value=env_doc.get("openai_model", "gpt-4o-mini"))
            env_doc["embedding_model"] = st.text_input(lang["admin_env_embedding_model"],
                                                       value=env_doc.get("embedding_model", "text-embedding-3-small"))

            env_doc["pinecone_api_key"] = st.text_input(lang["admin_env_pinecone_api_key"],
                                                        value=env_doc.get("pinecone_api_key", ""), type="password")
            env_doc["pinecone_host"] = st.text_input(lang["admin_env_pinecone_host"],
                                                     value=env_doc.get("pinecone_host", ""))
            env_doc["pinecone_index_name"] = st.text_input(lang["admin_env_pinecone_index_name"],
                                                           value=env_doc.get("pinecone_index_name", ""))
            env_doc["pinecone_namespace"] = st.text_input(lang["admin_env_pinecone_namespace"],
                                                          value=env_doc.get("pinecone_namespace", ""))

            # ... (all existing RAG fields) ...
            env_doc["top_k"] = st.number_input(lang["admin_env_top_k"], 1, 100, int(env_doc.get("top_k", 5)))
            env_doc["temperature"] = st.slider(lang["admin_env_temperature"], 0.0, 2.0,
                                               float(env_doc.get("temperature", 0.2)), 0.1)
            env_doc["max_context_chars"] = st.number_input(lang["admin_env_max_context_chars"], 500, 100000,
                                                           int(env_doc.get("max_context_chars", 8000)), 500)
            env_doc["metadata_text_key"] = st.text_input(lang["admin_env_metadata_text_key"],
                                                         value=env_doc.get("metadata_text_key", "text"))
            env_doc["metadata_source_key"] = st.text_input(lang["admin_env_metadata_source_key"],
                                                           value=env_doc.get("metadata_source_key", "source"))
            env_doc["system_prompt"] = st.text_area(lang["admin_env_system_prompt"],
                                                    value=env_doc.get("system_prompt", DEFAULT_CHAT_SYSTEM_PROMPT),
                                                    height=150)

            # ... (all existing Mongo fields) ...
            env_doc["mongo_uri"] = st.text_input(lang["admin_env_mongo_uri"], value=env_doc.get("mongo_uri", ""))
            env_doc["mongo_db"] = st.text_input(lang["admin_env_mongo_db"], value=env_doc.get("mongo_db", "rag_chat"))
        ...
        if st.button(lang["admin_env_test_button"]):
            try:
                settings = AppSettings(
                    ...
                    auth_cookie_expiry_days=int(env_doc.get("auth_cookie_expiry_days"))
                )
                ...
                idx = get_pinecone_index(settings.pinecone_api_key, settings.pinecone_host,
        ...
        with st.form("add_user_form"):
            ...
            role = st.selectbox(lang["admin_users_form_role"],
                                [lang["admin_users_form_role_user"], lang["admin_users_form_role_admin"]], index=0)
            ...
            if st.form_submit_button(lang["admin_users_form_add_button"]):
                if not new_u.strip() or not new_p.strip() or not new_e.strip():
                    ...
                    st.success(lang["admin_users_form_add_success"].format(new_u=new_u)) if msg == "ok" else st.error(
                        f"{lang['admin_users_form_add_failed']}: {msg}")
        ...
        for u in db["users"].find({}).sort("username"):
            ...
            if u["username"] != "admin":
                if cols[4].button(lang["admin_users_list_delete_button"], key=f"del-{u['username']}", type="secondary"):
                    msg = delete_user(db, u["username"])
                    if msg == "ok":
                        st.success(lang["admin_users_list_delete_success"]); st.rerun()
                    else:
